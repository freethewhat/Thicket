...
**Architecture:** One minimal CI/release-pipeline change is required — `.goreleaser.yaml`'s `release:` block must set `prerelease: auto` (GoReleaser's default is `prerelease: false` regardless of tag shape, so without this a `vX.Y.Z-beta.N` tag would publish as the repo's latest stable release); once set, `release.yml`'s existing trigger on any `v*` tag needs no further change. All other work is client-side: `internal/update.LatestTag` gains a channel argument that switches between `GET .../releases/latest` (stable; GitHub excludes prereleases here by design) and `GET .../releases` (the newest-first list, which includes prereleases); `internal/update.IsNewer` gains prerelease-aware semver comparison so `-beta.N` tags sort correctly; `internal/tui` threads a channel through the existing update-check `tea.Cmd` and labels a prerelease toast as a beta offer; `scripts/install.sh` gets the same channel toggle for install/update-from-script; docs get one new…
...
- `.goreleaser.yaml` requires exactly one line (`release: prerelease: auto`) so a `vX.Y.Z-beta.N` tag is correctly flagged as a GitHub prerelease — GoReleaser's default is `prerelease: false` regardless of tag shape, which would otherwise publish a beta tag as the repo's latest stable release and push it to every default-channel user. No other CI/release-pipeline changes are needed; `.github/workflows/release.yml`'s existing trigger on any `v*` tag is untouched.
- No new dependencies.
