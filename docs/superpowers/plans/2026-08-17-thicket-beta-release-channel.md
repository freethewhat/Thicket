...
If any `-beta.N` tag already exists on the repo by this point, additionally run: `THICKET_CHANNEL=beta sh -c 'curl -fsSL https://api.github.com/repos/freethewhat/Thicket/releases | grep -m1 "\"tag_name\""'` and confirm it prints that tag. If no beta tag exists yet, skip this and note it as unverified until one is cut (the stable-path smoke test above already proves the grep/sed logic against real GitHub API output, and the beta branch is byte-identical except for the URL).
